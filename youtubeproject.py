...
st.set_page_config(page_title= "Youtube Data Harvesting and Warehousing | By Mugeshkumar",
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This app is created by *Mugeshkumar!*"""})
...
client = pymongo.MongoClient("mongodb+srv://mugeshk403:Lonelyrider16ce133@cluster0.skzzjcm.mongodb.net/?retryWrites=true&w=majority&appName=AtlasApp")                                                        #"mongodb+srv://mugeshk403:Lonelyrider16ce133@cluster0.skzzjcm.mongodb.net/?retryWrites=true&w=majority&appName=AtlasApp") #"mongodb+srv://mugeshk403:Lonelyrider16ce133@cluster0.iabwlcx.mongodb.net/?retryWrites=true&w=majority&appName=AtlasApp
db = client.Youtube_Data_Mugesh
...
api_key = "AIzaSyD8ixZ-OeggQdxYKda_hzGCRekTpG2glm4"
...
if selected == "View":
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        mycursor.execute("""SELECT Channel_name AS Channel_Name, Title AS Video_Title, Views AS View_count
                            FROM video_details
                            ORDER BY Views DESC
                            LIMIT 10""")
        df = pd.DataFrame(mycursor.fetchall(),columns=mycursor.column_names)
        ...
        mycursor.execute("""SELECT a.video_id AS Video_id, Title AS Video_Title, b.Total_Comments
                            ...
                            Inner JOIN (SELECT video_id,COUNT(comment_id) AS Total_Comments
        ...
        mycursor.execute("""SELECT channel_name AS Channel_Name,Title as Video_name,Likes AS Like_Count 
                            FROM video_details
                            ORDER BY Likes DESC
                            LIMIT 10""")
        df = pd.DataFrame(mycursor.fetchall(),columns=mycursor.column_names)
        ...
        mycursor.execute("""SELECT  Title as video_name, Likes AS Like_count
                            FROM video_details
                            ORDER BY Likes DESC""")
        df = pd.DataFrame(mycursor.fetchall(),columns=mycursor.column_names)
        st.write(df)
        ...
        mycursor.execute("""SELECT channel_name AS Channel_Name, Views as channel_views 
                            FROM channel_details
                            ORDER BY views DESC""")
        df = pd.DataFrame(mycursor.fetchall(),columns=mycursor.column_names)
        st.write(df)
